...
public class TaxaParcValidator implements ConstraintValidator<TaxaParcConstraint, Dados> {
	...
	public boolean isValid(Dados dados, ConstraintValidatorContext context) {
		if(dados.getTaxaParcelado() < calcTaxaParcMin(dados) || dados.getTaxaParcelado() > calcTaxaParcMax(dados))  {
			throw new ResponseStatusException(HttpStatus.INTERNAL_SERVER_ERROR, "As taxas parcelado devem estar entre " + df.format(calcTaxaParcMin(dados)) + "% e " + df.format(calcTaxaParcMax(dados)) + "%");
		}
		return true;
	}
	...
	public Float calcTaxaParcMin(Dados dados) {
		...
		}else return taxaParcMin + taxaMinAdicional(dados);
	...
	public Float calcTaxaParcMax(Dados dados) {
		if (dados.getFaturamento() <= 5000) {
			return taxaParcMax;
			}else return taxaParcMax + taxaMaxAdicional(dados);
	}
