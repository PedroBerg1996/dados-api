...
public class TaxaDebValidator implements ConstraintValidator<TaxaDebConstraint, Dados> {
	...
	public boolean isValid(Dados dados, ConstraintValidatorContext context) {
		if(dados.getTaxaDebito() < calcTaxaDebMin(dados) || dados.getTaxaDebito() > calcTaxaDebMax(dados)) {
			throw new ResponseStatusException(HttpStatus.INTERNAL_SERVER_ERROR, "As taxas debito devem estar entre " + df.format(calcTaxaDebMin(dados)) + "% e " + df.format(calcTaxaDebMax(dados)) + "%");
		}
		return true;
	}
	...
	public Float calcTaxaDebMin(Dados dados) {
		...
		}else return taxaDebMin + taxaMinAdicional(dados);
	...
	public Float calcTaxaDebMax(Dados dados) {
		if (dados.getFaturamento() <= 5000) {
			return taxaDebMax;
		}else return taxaDebMax + taxaMaxAdicional(dados);
	}
